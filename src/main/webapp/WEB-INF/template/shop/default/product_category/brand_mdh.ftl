		...
		<div class="list-content clearfix">
						...
						[@brand_list productCategoryId = rootProductCategory.id count = 8]
							[#if brands?has_content]
								[#list brands as brand]
										...
										<a [#if brandId==brand.id] style="background-color:red;" [/#if] href="${base}/product_category/brand/${brand.id}.jhtml?id=${brand.id}&productCategoryId=${rootProductCategory.id}">${brand.name}</a>
								[/#list]
							[/#if]
						[/@brand_list]
			...
			[/#list]		
				...
				<div class="content fr clearfix">
					...
					<div class="moods">
						<ul class="clearfix" >
							[@goods_list brandId = brand.id count = 30 orderBy = "monthSales desc"]
