...
**/	
  ...
  $enshrine.bind('click', function(event) {
      ...
      $.ajax({
        url: "/jshop/member/favorite/add.jhtml",
        type: "POST",
        data: {goodsId: id},
        dataType: "json",
        cache: false,
	...
	$(document).ajaxComplete(function(event, request, settings) {
		...
		if (tokenStatus == "accessDenied") {
			...
			if (url.indexOf("/jshop/register/") == 0 || url.indexOf("/jshop/login/") == 0 || url.indexOf("/jshop/cart/") == 0 || url.indexOf("/jshop/order/") == 0) {
				showHeaderCartQuantity();
			}
		}
	});
