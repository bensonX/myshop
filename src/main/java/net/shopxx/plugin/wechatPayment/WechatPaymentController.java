 ...
 * @author JSHOP Team
 \* @version 3.X
 */
@Controller("adminWechatPaymentController")
@RequestMapping("/admin/payment_plugin/wechat_payment")
public class WechatPaymentController extends BaseController {

	@Resource(name = "wechatPaymentPlugin")
	private WechatPaymentPlugin wechatPaymentPlugin;
	...
	public @ResponseBody
	Message install() {
		if (!wechatPaymentPlugin.getIsInstalled()) {
			PluginConfig pluginConfig = new PluginConfig();
			pluginConfig.setPluginId(wechatPaymentPlugin.getId());
	...
	Message uninstall() {
		if (wechatPaymentPlugin.getIsInstalled()) {
			pluginConfigService.deleteByPluginId(wechatPaymentPlugin.getId());
		}
		return SUCCESS_MESSAGE;
	}
	...
	public String setting(ModelMap model) {
		...
		return "/net/shopxx/plugin/wechatPayment/setting";
	}
